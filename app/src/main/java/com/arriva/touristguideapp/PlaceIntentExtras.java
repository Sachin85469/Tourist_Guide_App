...
/**
 * Writes {@link PlaceDetailsActivity} intent extras consistently (remote URLs, gallery URLs, drawable fallbacks).
 */
public final class PlaceIntentExtras {
    ...
    public static void putPlaceDetails(@NonNull Intent intent, @NonNull Place place) {
        ...
        if (place.getDrawableAssetKey() != null && !place.getDrawableAssetKey().trim().isEmpty()) {
            intent.putExtra("drawableAssetKey", place.getDrawableAssetKey().trim());
        }
        ArrayList<String> gKeys = new ArrayList<>();
        for (String k : place.getGalleryDrawableKeys()) {
            if (k != null && !k.trim().isEmpty()) {
                gKeys.add(k.trim());
            }
        }
        if (!gKeys.isEmpty()) {
            intent.putStringArrayListExtra("galleryDrawableKeys", gKeys);
        }
    }
}
